...
public class ModifyProductController implements Initializable, ControllerInterface {
    
    // Declare fields
    private ObservableList<Part> addParts = FXCollections.observableArrayList();
    private int productId;
    private final int productIndex = getModifyProductIndex();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        //method will be updated on deploy
    }
    ...
    void handleSaveProduct(ActionEvent event) throws IOException {
        ...
        try {
            String errorMessage=ProductValidator.validateProduct(name, Double.parseDouble(price), Integer.parseInt(inStock), Integer.parseInt(min), Integer.parseInt(max), addParts);
            if(errorMessage.isEmpty()) {
                showAlert("Error Modifying Part!", errorMessage);
            } else {
            ...
            showAlert("Error Modifying Part!", "Form contains blank or invalid fields.");
